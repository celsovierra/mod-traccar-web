...
const SmsMarketModal = ({ device, onClose }) => {
  ...
  const handleSend = async (type, phoneOverride = '') => {
    ...
    if (type === 'GPRS') {
      ...
      if ((device?.status || '').toLowerCase() !== 'online') {
        alert('Este veículo está offline. Use SMS para enviar o comando.');
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(detail || 'O Traccar não aceitou o comando GPRS.');
        }
        ...
        setMessage('');
        alert('Comando enviado via GPRS para o veículo online.');
      } catch (error) {
    ...
    if (!creds?.user?.trim() || !creds?.pass?.trim()) {
      alert('Configure o usuário e a senha do ENVIAR COMANDO na engrenagem.');
  ...
  return (
    <Dialog className="sms-market-modal" open={true} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: '18px', m: { xs: 1, sm: 2 }, maxHeight: '92vh', overflow: 'hidden' } }}>
      ...
      <Tabs value={tabValue} onChange={(e, value) => setTabValue(value)} variant="fullWidth" sx={{ borderBottom: 1, borderColor: 'divider', px: { xs: 0, sm: 2 }, pt: 1, bgcolor: '#fff', '& .MuiTab-root': { minHeight: 68, fontSize: { xs: '0.65rem', sm: '0.75rem' }, fontWeight: 'bold' }, '& .Mui-selected': { color: '#1565c0' } }}>
        ...
        {tabValue === 2 ? (
          ...
          <>
            {tabValue === 3 && (
              <>
                ...
                <TextField
                  ...
                  disabled={sending}
                  sx={{ mb: 1.5, bgcolor: '#fff', '& .MuiOutlinedInput-root': { borderRadius: '10px' } }}
                />

                <Box sx={{ mb: 2, p: 1.5, border: '1px solid #dbe7f5', borderRadius: '10px', bgcolor: '#f8fbff' }}>
                  <Typography variant="caption" fontWeight="bold" color="#1976d2" display="block" mb={1}>
                    ...
                    <Box>
                      ...
                      {savedGroups.length > 0 ? savedGroups.map((group) => (
                        <Button
                          ...
                          sx={{ mb: 0.8, justifyContent: 'flex-start', textTransform: 'none', bgcolor: '#1976d2' }}
                        >
            ...
            <TextField
              ...
              multiline
              rows={3}
              placeholder={tabValue === 3 ? 'Digite a mensagem SMS...' : 'Digite o comando ou mensagem...'}
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              variant="outlined"
              disabled={sending}
              sx={{ mb: 2.5, bgcolor: '#fff', '& .MuiOutlinedInput-root': { borderRadius: '10px' } }}
            />
          </>
        )}
